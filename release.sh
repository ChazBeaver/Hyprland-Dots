...
if [ -f Hyprland-Dots.tar.gz ]; then
  ...
  latest_version=$(curl -s https://api.github.com/repos/chaz/Hyprland-Dots/releases/latest | grep "tag_name" | cut -d '"' -f 4 | sed 's/v//')
  ...
  if [ "$existing_version" = "$latest_version" ]; then
    ...
    if [ "$upgrade_choice" = "y" ]; then
		...
		# Delete existing directories starting with chaz-Hyprland-Dots
      find . -type d -name 'chaz-Hyprland-Dots*' -exec rm -rf {} +
...
latest_tag=$(curl -s https://api.github.com/repos/chaz/Hyprland-Dots/releases/latest | grep "tag_name" | cut -d '"' -f 4)
...
latest_tarball_url=$(curl -s https://api.github.com/repos/chaz/Hyprland-Dots/releases/latest | grep "tarball_url" | cut -d '"' -f 4)
...
if curl -L "$latest_tarball_url" -o "$file_name"; then
  ...
  rm -rf chaz-Hyprland-Dots

  # Identify the extracted directory
  extracted_directory=$(tar -tf "$file_name" | grep -o '^[^/]\+' | uniq)

  # Rename the extracted directory to chaz-Hyprland-Dots
  mv "$extracted_directory" chaz-Hyprland-Dots || exit 1

  cd "chaz-Hyprland-Dots" || exit 1
